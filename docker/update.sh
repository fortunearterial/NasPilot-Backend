...
function download_and_unzip() {
    ...
    while [ $retries -lt $max_retries ]; do
        if curl ${CURL_OPTIONS} "${url}" ${CURL_HEADERS} | busybox unzip -d ${TMP_PATH} - > /dev/null; then
            if [ -e ${TMP_PATH}/NasPilot-* ]; then
                mv ${TMP_PATH}/NasPilot-* ${TMP_PATH}/"${target_dir}"
            fi
            break
        else
...
function install_backend_and_download_resources() {
    # 更新后端程序
    if ! download_and_unzip "${GITHUB_PROXY}https://github.com/fortunearterial/NasPilot/archive/refs/${1}" "App"; then
    ...
    if [[ "${1}" == "heads/v2.zip" ]]; then
        ...
        releases=$(curl ${CURL_OPTIONS} "https://api.github.com/repos/fortunearterial/NasPilot-Frontend/releases" ${CURL_HEADERS} | jq -r '.[].tag_name' | grep "^v2\.")
        if [ -z "$releases" ]; then
...
function test_connectivity_github() {
    ...
    0)
        if [[ -n "${GITHUB_PROXY}" ]]; then
            if curl -sL "${GITHUB_PROXY}https://raw.githubusercontent.com/fortunearterial/NasPilot/main/README.md" > /dev/null 2>&1; then
                GITHUB_LOG="镜像代理模式"
                return 0
            fi
        fi
    ...
    1)
        if [[ -n "${PROXY_HOST}" ]]; then
            if curl -sL -x ${PROXY_HOST} https://raw.githubusercontent.com/fortunearterial/NasPilot/main/README.md > /dev/null 2>&1; then
...
if [[ "${NASPILOT_AUTO_UPDATE}" = "true" ]] || [[ "${NASPILOT_AUTO_UPDATE}" = "release" ]] || [[ "${NASPILOT_AUTO_UPDATE}" = "dev" ]]; then
    ...
    if [ "${NASPILOT_AUTO_UPDATE}" = "dev" ]; then
        ...
        if [[ "${old_version}" == *APP_VERSION* ]]; then
            ...
            # 获取所有发布的版本列表，并筛选出以v2开头的版本号
            releases=$(curl ${CURL_OPTIONS} "https://api.github.com/repos/fortunearterial/NasPilot/releases" ${CURL_HEADERS} | jq -r '.[].tag_name' | grep "^v2\.")
            if [ -z "$releases" ]; then
    ...
    INFO "程序自动升级已关闭，如需自动升级请在创建容器时设置环境变量：NASPILOT_AUTO_UPDATE=release"
else
    INFO "NASPILOT_AUTO_UPDATE 变量设置错误"
fi

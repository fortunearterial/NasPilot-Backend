...
def is_valid_episode_number(number):
    return str2float(number) <= EPISODE_NUMBER_MAX
...
def set_episode_number(number, token, validate):
    ...
    if Elements.get_check_alt_number():
        ...
        if str2float(number) > str2float(episode_number):
            category = ElementCategory.EPISODE_NUMBER_ALT
        elif str2float(number) < str2float(episode_number):
...
def number_comes_before_another_number(token):
    ...
    if separator_token:
        ...
        if separator == '&' or separator == 'of':
            ...
            if other_token and is_number(other_token.content):
...
def search_for_episode_patterns(tokens):
    for token in tokens:
        numeric_front = is_number(token.content[0])
...
def match_episode_patterns(word, token):
    ...
    numeric_front = is_number(word[0])
    numeric_back = is_number(word[-1])
...
def is_valid_volume_number(number):
    return float(number) <= VOLUME_NUMBER_MAX
...
def search_for_equivalent_numbers(tokens):
    for token in tokens:
        ...
        if not parser_helper.is_token_isolated(next_token) or \
                not is_number(next_token.content) or \
                not is_valid_episode_number(next_token.content):
            continue

        episode = min(token, next_token, key=lambda t: float(t.content))
        alt_episode = max(token, next_token, key=lambda t: float(t.content))

...
class Parser:
    ...
    def search_for_keywords(self):
        for token in Tokens.get_list(TokenFlags.UNKNOWN):
            ...
            if len(word) != 8 and self.is_number(word):
                continue
    ...
    def search_for_isolated_numbers(self):
        for token in Tokens.get_list(TokenFlags.UNKNOWN):
            if not self.is_number(token.content) or \
                    not parser_helper.is_token_isolated(token):
                continue

            number = float(token.content)
    ...
    def search_for_episode_number(self):
        ...
        tokens = [token for token in tokens if self.is_number(token.content)]

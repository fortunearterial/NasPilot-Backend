...
class ServiceConfigHelper:
    ...
    @staticmethod
    def get_user_configs(user_id: int, config_key: UserConfigKey, conf_type: Type) -> List:
        """
        通用获取配置的方法，根据 config_key 获取相应的配置并返回指定类型的配置列表

        :param user_id: 用户ID
        :param config_key: 用户配置的 key
        :param conf_type: 用于实例化配置对象的类类型
        :return: 配置对象列表
        """
        config_data = UserConfigOper().get(user_id, config_key)
        if not config_data:
            return []
        # 直接使用 conf_type 来实例化配置对象
        return [conf_type(**conf) for conf in config_data]
    ...
    @staticmethod
    def get_downloader_configs(user_id: Optional[int] = None) -> List[DownloaderConf]:
        """
        获取下载器的配置
        """
        return ServiceConfigHelper.get_user_configs(user_id, UserConfigKey.Downloaders, DownloaderConf)

    @staticmethod
    def get_mediaserver_configs(user_id: Optional[int] = None) -> List[MediaServerConf]:
        """
        获取媒体服务器的配置
        """
        return ServiceConfigHelper.get_user_configs(user_id, UserConfigKey.MediaServers, MediaServerConf)

    @staticmethod
    def get_notification_configs(user_id: Optional[int] = None) -> List[NotificationConf]:
        """
        获取消息通知渠道的配置
        """
        return ServiceConfigHelper.get_user_configs(user_id, UserConfigKey.Notifications, NotificationConf)

    @staticmethod
    def get_notification_switches(user_id: Optional[int] = None) -> List[NotificationSwitchConf]:
        """
        获取消息通知场景的开关
        """
        return ServiceConfigHelper.get_user_configs(user_id, UserConfigKey.NotificationSwitchs, NotificationSwitchConf)

    @staticmethod
    def get_notification_switch(mtype: NotificationType, user_id: Optional[int] = None) -> Optional[str]:
        """
        获取指定类型的消息通知场景的开关
        """
        switchs = ServiceConfigHelper.get_notification_switches(user_id)
...
class UserServiceBaseHelper(Generic[TConf]):
    """
    通用服务帮助类，抽象获取配置和服务实例的通用逻辑
    """

    def __init__(self, config_key: UserConfigKey, conf_type: Type[TConf], module_type: ModuleType):
        self.modulemanager = ModuleManager()
        self.config_key = config_key
        self.conf_type = conf_type
        self.module_type = module_type

    def get_configs(self, user_id: int, include_disabled: bool = False) -> Dict[str, TConf]:
        """
        获取配置列表
    ...
    def get_config(self, user_id: int, name: str) -> Optional[TConf]:
        """
        获取指定名称配置
        """
        if not name:
    ...
    def iterate_module_instances(self, user_id: int) -> Iterator[ServiceInfo]:
        """
        迭代所有模块的实例及其对应的配置，返回 ServiceInfo 实例
        """
        configs = self.get_configs(user_id)
    ...
    def get_services(self, type_filter: Optional[str] = None, name_filters: Optional[List[str]] = None) \
        ...
        return {
            service_info.name: service_info
            for service_info in self.iterate_module_instances(user_id)
            if service_info.config and (
                    type_filter is None or service_info.type == type_filter
            ) and (
                       name_filters_set is None or service_info.name in name_filters_set)
    ...
    def get_service(self, user_id: int, name: str, type_filter: Optional[str] = None) -> Optional[ServiceInfo]:
        """
        获取指定名称的服务信息，并根据类型过滤

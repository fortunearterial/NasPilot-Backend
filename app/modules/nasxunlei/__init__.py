...
from app.modules.nasxunlei.nasxunlei import NasXunlei
...
class NasXunleiModule(_ModuleBase):
    nasxunlei: NasXunlei = None
    ...
    def scheduler_job(self) -> None:
        ...
        """
        # 定时重连
        if self.nasxunlei.is_inactive():
            self.nasxunlei.reconnect()
    ...
    def download(self, content: Union[Path, str], download_dir: Path, cookie: str,
        ...
        # 添加任务
        state = self.nasxunlei.add_torrent(
        ...
        if not state:
            ...
            # 查询所有下载器的种子
            torrents, error = self.nasxunlei.get_torrents()
            if error:
                return None, "无法连接nasxunlei下载器"
            if torrents:
                for torrent in torrents:
                    ...
                    if torrent.get("name") == torrent_name and torrent.get("total_size") == torrent_size:
                        ...
                        if "已整理" in torrent_tags:
                            self.nasxunlei.remove_torrents_tag(ids=torrent_hash, tag=['已整理'])
                        if settings.TORRENT_TAG and settings.TORRENT_TAG not in torrent_tags:
                            ...
                            self.nasxunlei.set_torrents_tag(ids=torrent_hash, tags=[settings.TORRENT_TAG])
                        return torrent_hash, f"下载任务已存在"
            return None, f"添加种子任务失败：{content}"
        else:
            # 获取种子Hash
            torrent_hash = self.nasxunlei.get_torrent_id_by_tag(tags=tag)
            if not torrent_hash:
                return None, f"获取种子Hash失败：{content}"
            else:
                if is_paused:
                    # 种子文件
                    torrent_files = self.nasxunlei.get_files(torrent_hash)
                    ...
                    if sucess_epidised and file_ids:
                        # 选择文件
                        self.nasxunlei.set_files(torrent_hash=torrent_hash, file_ids=file_ids, priority=0)
                    # 开始任务
                    if settings.QB_FORCE_RESUME:
                        # 强制继续
                        self.nasxunlei.torrents_set_force_start(torrent_hash)
                    else:
                        self.nasxunlei.start_torrents(torrent_hash)
    ...
    def list_torrents(self, status: TorrentStatus = None,
        ...
        if hashs:
            # 按Hash获取
            torrents, _ = self.nasxunlei.get_torrents(ids=hashs, tags=settings.TORRENT_TAG)
            ...
            # 获取已完成且未整理的
            torrents = self.nasxunlei.get_completed_torrents(tags=settings.TORRENT_TAG)
            for torrent in torrents or []:
            ...
            # 获取正在下载的任务
            torrents = self.nasxunlei.get_downloading_torrents(tags=settings.TORRENT_TAG)
            for torrent in torrents or []:
    ...
    def transfer_completed(self, hashs: Union[str, list],
        ...
        :param path:  源目录
        """
        self.nasxunlei.set_torrents_tag(ids=hashs, tags=['已整理'])
        # 移动模式删除种子
    ...
    def remove_torrents(self, hashs: Union[str, list]) -> bool:
        ...
        :return: bool
        """
        return self.nasxunlei.delete_torrents(delete_file=True, ids=hashs)
    ...
    def start_torrents(self, hashs: Union[list, str]) -> bool:
        ...
        :return: bool
        """
        return self.nasxunlei.start_torrents(ids=hashs)
    ...
    def stop_torrents(self, hashs: Union[list, str]) -> bool:
        ...
        :return: bool
        """
        return self.nasxunlei.stop_torrents(ids=hashs)

    def torrent_files(self, tid: str) -> Optional[SimpleNamespace]:
        """
        获取种子文件列表
        """
        return self.nasxunlei.get_files(tid=tid)
    ...
    def downloader_info(self) -> schemas.DownloaderInfo:
        ...
        info = self.nasxunlei.transfer_info()
        if not info:

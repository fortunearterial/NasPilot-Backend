...
class SteamCache(metaclass=Singleton):
    """
    Steam缓存数据
    {
        "id": '',
    ...
    # STEAM缓存过期
    _steam_cache_expire: bool = False

    def __init__(self):
        self._meta_path = settings.TEMP_PATH / "__steam_cache__"
    ...
    def __get_key(meta: MetaBase) -> str:
        ...
        获取缓存KEY
        """
        return f"[{meta.type.value if meta.type else '游戏'}]{meta.name}-{meta.year}"
    ...
    def get(self, meta: MetaBase):
        ...
        with lock:
            ...
            if info:
                ...
                elif expire and self._steam_cache_expire:
                    self.delete(key)
            return info or {}
    ...
    def delete_by_steamid(self, steamid: str) -> None:
        """
        清空对应STEAM ID的所有缓存记录，以强制更新STEAM中最新的数据
        """
        for key in list(self._meta_data):
            if self._meta_data.get(key, {}).get("id") == steamid:
                with lock:
                    self._meta_data.pop(key)

    def delete_unknown(self) -> None:
        """
        清除未识别的缓存记录，以便重新搜索STEAM
        """
        for key in list(self._meta_data):
    ...
    def update(self, meta: MetaBase, info: dict) -> None:
        ...
        with lock:
            if info:
                ...
                # 缓存年份
                cache_year = info.get("release_date").get("date")[:4] if info.get("release_date") else None
                # 类型
                if isinstance(info.get('media_type'), MediaType):
                    mtype = info.get('media_type')
                elif info.get("type"):
                    mtype = MediaType.GAME if info.get("type") == "game" else MediaType.UNKNOWN
                else:
                    meta = MetaInfo(cache_title)
                    mtype = MediaType.GAME
                # 海报
                poster_path = info.get("header_image")
                if not poster_path and info.get("capsule_image"):
                    poster_path = info.get("capsule_image")
                if not poster_path and info.get("capsule_imagev5"):
                    poster_path = info.get("capsule_imagev5")

                self._meta_data[self.__get_key(meta)] = {
                        "id": info.get("steam_appid"),
                        "type": mtype,
                        "year": cache_year,
                        "title": cache_title,

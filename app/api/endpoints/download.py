...
def current(
    ...
    查询正在下载的任务
    """
    return DownloadChain().downloading(current_user.id, name)
...
def start(
    ...
    开如下载任务
    """
    ret = DownloadChain().set_downloading(current_user.id, hashString, "start")
...
def stop(hashString: str,
    ...
    暂停下载任务
    """
    ret = DownloadChain().set_downloading(current_user.id, hashString, "stop")
    return schemas.Response(success=True if ret else False)
...
def clients(current_user: schemas.User = Depends(get_current_user)) -> Any:
    ...
    查询可用下载器
    """
    downloaders: List[dict] = UserConfigOper().get(current_user.id, UserConfigKey.Downloaders)
    if downloaders:
...
def delete(hashString: str,
    ...
    删除下载任务
    """
    ret = DownloadChain().remove_downloading(current_user.id, hashString)
    return schemas.Response(success=True if ret else False)
